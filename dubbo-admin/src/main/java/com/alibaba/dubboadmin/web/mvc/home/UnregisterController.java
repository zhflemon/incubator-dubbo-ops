	...
	protected String doExecute(Map<String, Object> context) throws Exception {
		...
		if (params == null || params.size() == 0) {
			throw new IllegalArgumentException(
					"The url parameters is null! Usage: " + request.getRequestURL().toString()
							+ "?com.xxx.XxxService=http://" + operatorAddress + "/xxxService");
		}
			...
			if (entry.getKey() != null && entry.getKey().length() > 0 && entry.getValue() != null
				...
				if (!currentUser.hasServicePrivilege(entry.getKey())) {
					throw new IllegalStateException(
							"The user " + operator + " have no privilege of service " + entry.getKey());
				}
